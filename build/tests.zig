...
//! GPU abstraction spec, and the composite `test-tdd` suite.
...
pub fn create(b: *std.Build, target: std.Build.ResolvedTarget, optimize: std.builtin.OptimizeMode, m: Modules) TestSteps {
    ...
    const tdd = b.step("test-tdd", "Run the framework's behavioral suite (integration + gpu; needs a display + Vulkan)");
